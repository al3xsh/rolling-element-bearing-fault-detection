...
results_dir = ('./weights/wdcnn_snr_clr/' +
               'cross_validation_{0}/').format(time.strftime("%Y%m%d_%H%M"))
...
print('results for the wdcnn model')
...
with open('{0}parameters.txt'.format(results_dir), 'a') as f:
    f.write('\n')
    f.write('results for the wdcnn model\n')
